...
#ifdef DEBUG
	slcd_selLog->Log('T', "lcdc_Init() -> %p", lcd);
#endif
...
static int lcdl_Init(lua_State *L){
	...
	struct SelLCDScreenLua *lcd = (struct SelLCDScreenLua *)lua_newuserdata(L, sizeof(struct SelLCDScreenLua));
	assert(lcd);
	lcd->storage = lcds;
...
#ifdef DEBUG
	slcd_selLog->Log('T', "lcdc_Init() -> lcd : %p, lua: %p", lcd->storage, lcd);
#endif
 ...
 */
#ifdef DEBUG
	slcd_selLog->Log('T', "lcdc_Shutdown(%p)", lcd);
#endif
 ...
 */
#ifdef DEBUG
	slcd_selLog->Log('T', "lcdc_SetCursor(%p, %d, %d)", lcd, x,y);
#endif
 ...
 */
#ifdef DEBUG
	slcd_selLog->Log('T', "lcdc_SetCGRAM(%p, %d)", lcd, pos);
#endif
...
static void lcdc_bWriteString(struct SelLCDScreen *lcd, const char *atxt){
/** 
...
bool InitModule( void ){
	...
	slcd_selLCD.bClear = lcdc_bClear;
