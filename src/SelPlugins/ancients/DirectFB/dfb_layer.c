...
static int LayerCreateWindow(lua_State *L){
	...
	if(lua_type(L, -1) == LUA_TNUMBER){
		...
		dsc.surface_caps = luaL_checkinteger(L, -1);
	}
	lua_pop(L, 1);	/* cleaning ... */
	...
	if(lua_type(L, -1) == LUA_TNUMBER){
		...
		dsc.options = luaL_checkinteger(L, -1);
	}
	lua_pop(L, 1);	/* cleaning ... */
	...
	if(lua_type(L, -1) == LUA_TNUMBER){
		...
		dsc.stacking = luaL_checkinteger(L, -1);
	}
	lua_pop(L, 1);	/* cleaning ... */
	...
	if(lua_type(L, -1) == LUA_TNUMBER){
		...
		dsc.posx = luaL_checkinteger(L, -1);
	}
	lua_pop(L, 1);	/* cleaning ... */
	...
	if(lua_type(L, -1) == LUA_TNUMBER){
		...
		dsc.posy = luaL_checkinteger(L, -1);
	}
	lua_pop(L, 1);	/* cleaning ... */
...
void _include_SelLayer( lua_State *L ){
	libSel_objFuncs( L, "SelLayer", SelLayerM );
	libSel_libFuncs( L, "SelLayer", SelLayerLib );
}
