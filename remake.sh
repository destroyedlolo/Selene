...
LFMakeMaker -v +f=Makefile -cc='gcc -Wall -DUSE_DIRECTFB -DUSE_MQTT -DPLUGIN_DIR=\".\" -DxDEBUG -std=c99 `directfb-config --cflags` `directfb-config --libs` `pkg-config --cflags lua` `pkg-config --libs lua` -lpaho-mqtt3c -ldl ' *.c -t=../Selene > Makefile
